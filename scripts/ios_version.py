...
def sync_plist(path, version, build_version=None, release_version=None):
    """Update only version fields, preserving application identity and settings."""
    with path.open("rb") as source:
        info = plistlib.load(source)
    info["CFBundleShortVersionString"] = version
    info["CFBundleVersion"] = build_version or version
    if release_version is not None:
        info["InverterReleaseVersion"] = release_version
    with path.open("wb") as destination:
        plistlib.dump(info, destination, sort_keys=False)


def verify_ipa(path, version, build_version=None, release_version=None):
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        if release_version is not None:
            expected["InverterReleaseVersion"] = release_version
...
def main():
    ...
    parser.add_argument(
        "--config", type=Path, default=Path("src-tauri/tauri.conf.json")
    )
    args = parser.parse_args()
    validate_version(args.version, args.config)
    build_version = release_version = None
    if args.plan:
        # Legacy local iOS checks need no release-plan tooling unless requested.
        # pylint: disable-next=import-outside-toplevel
        from version_plan import validate_plan, projected_value

        plan = validate_plan(json.loads(args.plan.read_text(encoding="utf-8")))
        if plan["base_version"] != args.version:
            raise ValueError("iOS version does not match release plan")
        build_version = projected_value(plan, {"value": "apple-build"})
        release_version = plan["version"]
    if args.mode == "sync":
        sync_plist(args.path, args.version, build_version, release_version)
    else:
        verify_ipa(args.path, args.version, build_version, release_version)
