#!/usr/bin/env python3
"""Stage one prebuilt desktop worker for the native plugin-package signer.
...
def prepare(worker, target, output, plugin="frigate"):
    """Create a new staging directory containing only the supplied regular file."""
    if plugin not in PLUGINS:
        raise ValueError("Choose a supported built-in plugin")
    template_name, binary_name = PLUGINS[plugin]
    if target not in TARGETS:
        raise ValueError("Only supported desktop targets can be packaged")
    ...
    try:
        ...
        name = binary_name + (".exe" if "windows" in target else "")
...
def main(default_plugin=None):
    """Require an explicit prebuilt worker and its desktop compilation target."""
    parser = argparse.ArgumentParser(description=__doc__)
    if default_plugin is None:
        parser.add_argument(
            "--plugin", choices=PLUGINS, required=True, help="Built-in package metadata"
        )
    else:
        parser.set_defaults(plugin=default_plugin)
    ...
    try:
        prepare(args.worker, args.target, args.output, args.plugin)
    except (OSError, ValueError) as error:
        parser.exit(1, f"Cannot prepare plugin package: {error}\n")
    print(
