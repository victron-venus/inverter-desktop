...
async fn session(
    ...
    tokio::select! {
        ...
        result=inverter_camera_common::network::run(configuration.broker()?, events::Events::new(&configuration)?, output)=>result,
    }
}
...
fn main() -> ExitCode {
    std::panic::set_hook(Box::new(|_| eprintln!("Kerberos worker failed")));
        ...
        .build()
    {
        Ok(runtime) => runtime,
        Err(_) => {
            eprintln!("Kerberos worker could not start");
    ...
    if result.is_err() {
        eprintln!("Kerberos worker session failed");
        ExitCode::FAILURE
    } else {
        ExitCode::SUCCESS
    }
