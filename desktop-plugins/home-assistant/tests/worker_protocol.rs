...
impl Worker {
    ...
    fn hello(&mut self) {
        ...
        assert_eq!(
            self.next(),
            json!({"type":"ready","protocol_version":1,"host_api_version":"1.3.0",
...
fn hello() -> Value {
    json!({"type":"hello","protocol_version":1,"host_api_version":"1.3.0",
        "plugin_id":"inverter-desktop.home-assistant"})
}
...
fn strict_handshake_rejects_wrong_identity_protocol_api_and_unknown_fields() {
    ...
    for (key, value) in [
        ...
        ("host_api_version", json!("1.3.0-beta.1")),
...
fn blocked_output(eof: bool) {
    ...
    let mut frame = hello();
    frame["host_api_version"] = json!("1.3.0+");
    let padding = MAX_FRAME - serde_json::to_vec(&frame).unwrap().len() - 1;
    frame["host_api_version"] = json!(format!("1.3.0+{}", "a".repeat(padding)));
