<template>
  <div class="flex flex-col gap-1 h-full">
    <!-- EV Section -->
    <div v-if="features?.ev !== false && showEv !== false" class="classic-card">
      ...
      <div class="p-1 flex justify-between items-center gap-2">
        ...
        <div class="text-center" v-if="parseFloat(evPower) > 0">
          ...
          <div class="text-[10px] text-slate-500 font-bold tracking-tighter">VUE</div>
        </div>
        <div class="text-right">
          <div class="text-2xl font-bold text-accent leading-none">
            {{ Math.floor(carSoc || 0) }}%
          </div>
          <div class="text-[10px] text-slate-500 font-bold text-center tracking-tighter">SoC</div>
        </div>
      </div>
    </div>
    ...
    <div
      v-if="
        (showDishwasher !== false && dishwasherRunning) ||
        (showWasher !== false && ((washerTime || 0) > 0 || washerPower)) ||
        (showDryer !== false && ((dryerTime || 0) > 0 || dryerPower))
      "
      class="flex flex-col gap-0.5"
    >
      <div
        v-if="showDishwasher !== false && dishwasherRunning"
      ...
      <div
        v-if="showWasher !== false && ((washerTime || 0) > 0 || washerPower)"
        class="classic-card px-2 py-0.5 flex justify-between items-center"
      >
      ...
      <div
        v-if="showDryer !== false && ((dryerTime || 0) > 0 || dryerPower)"
        class="classic-card px-2 py-0.5 flex justify-between items-center"
      >
...
import { Car, Droplets, Home as HomeIcon } from 'lucide-vue-next'
