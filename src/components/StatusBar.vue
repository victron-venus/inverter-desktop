<template>
  <div class="flex items-center justify-center gap-2 text-[11px] font-medium text-slate-400 mt-1.5 pb-1">
    ...
    <div class="flex items-center gap-1">
      ...
      <span class="text-slate-800 dark:text-white font-bold">{{ formatUptime(uptime || 0) }}</span>
    </div>
    ...
    <div class="flex items-center gap-1">
      ...
      <span class="font-bold text-slate-800 dark:text-white">MQTT</span>
    </div>

    <span class="opacity-30 mx-0.5">|</span>
    <span class="text-slate-600 dark:text-slate-400 font-medium">Desktop {{ appVersion }}</span>

    <span v-if="stateVersion" class="opacity-30 mx-0.5">|</span>
    <span v-if="stateVersion" class="text-slate-600 dark:text-slate-400 font-medium">Control {{ stateVersion }}</span>
  </div>
</template>
