<template>
  <div class="flex flex-col gap-2">
    ...
    <div v-if="haEntitiesList.length === 0" class="py-4 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded text-[11px] text-slate-600 bg-slate-50/30 dark:bg-slate-900/30">
      ...
      <div v-for="(entity, index) in haEntitiesList" :key="entity.id || `home-${index}`" 
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="flex items-center gap-0.5 pt-3">
            ...
            <button @click="$emit('move-down', index)" :disabled="index === haEntitiesList.length - 1" class="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 disabled:opacity-20 text-slate-600">
              <ChevronDown :size="14" />
            </button>
        ...
        <div class="flex items-center gap-4 px-1 border-t border-slate-50 dark:border-slate-800/50 pt-1.5">
          <label class="flex items-center gap-2 cursor-pointer group">
            ...
            <span class="text-[9px] font-bold uppercase text-slate-600 group-hover:text-accent transition-colors">Active</span>
          </label>
        </div>
      </div>
    </div>
