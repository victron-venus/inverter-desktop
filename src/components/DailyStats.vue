<template>
  <div class="p-0 mb-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[12px] font-medium leading-tight text-slate-700 dark:text-slate-300">
    ...
    <div v-if="hasGrid" class="flex items-center gap-1.5 mr-1">
      ...
      <span class="text-slate-800 dark:text-slate-400 uppercase text-[10px] font-bold tracking-tighter">Grid:</span>
      <span class="font-bold text-slate-950 dark:text-slate-100">{{ grid }}kWh</span>
    ...
    <div v-if="hasBattery" class="flex items-center gap-1.5 flex-1 min-w-fit">
      ...
      <div class="flex items-center gap-1.5">
        <span class="text-slate-800 dark:text-slate-400 uppercase text-[10px] font-bold tracking-tighter">I:</span>
        <span class="font-bold text-slate-950 dark:text-slate-100">{{ batIn }}kWh</span>
        <span class="text-slate-600 dark:text-slate-400 text-[10px]">({{ batInY }})</span>
        
        <span class="text-slate-800 dark:text-slate-400 uppercase text-[10px] font-bold tracking-tighter ml-0.5">O:</span>
        <span class="font-bold text-slate-950 dark:text-slate-100">{{ batOut }}kWh</span>
        <span class="text-slate-600 dark:text-slate-400 text-[10px]">({{ batOutY }})</span>
...
const solarStr = computed(() => {
  ...
  const mpptSum = mpptDaily.value.reduce((a, b) => a + b, 0)
  parts.push(pvTotalDaily.value.toFixed(2) + '(' + mpptSum.toFixed(2) + ')')
  return parts.join('+')
})
